...
涉及复杂 Node.js 架构模式，并且当前环境没有等价能力时再安装：
...
反例测试必须先运行并确认失败原因正确，再允许实现。测试因语法、依赖、环境或配置错误而失败不算 RED；测试一开始就通过时，必须修正测试或恢复到修改前基线。测试不应该只验证 mock 调用次数，而要验证真实业务结果、持久化结果、错误响应或外部事件语义。

已有半成品实现不能保留作参考后反向补测试。不得擅自删除、回退、覆盖、移动、stash 或清理用户已有及未提交改动；优先在新 worktree、临时副本或其他不改变当前工作区状态的基线上建立 RED。任何会改变当前工作区状态的隔离操作都必须先取得用户确认。项目没有测试体系时，先建立当前任务所需的最小测试入口。一次性原型、生成代码或纯配置变更只有经用户明确同意才可例外；Bug 修复、赶时间和改动很小不是例外。

进入 GREEN 前必须报告 RED 的测试命令、退出状态和关键断言失败摘要。
...
- 任务达到按需条件时已加载 nodejs-backend-patterns；普通初始化无需安装或提示。
- Swagger/OpenAPI 已接入。
- 每个接口有入参和响应类型。
- Zod 统一校验输入。
- 校验错误和业务错误结构统一。
...
- 功能、修复、重构和行为变化至少有一个已经正确失败过的反例测试；允许的例外有用户明确确认记录。
- AI PR 检查清单已完成。
